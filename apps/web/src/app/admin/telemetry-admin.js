...
/**
 * TelemetryAdminController — gestão de telemetria local, auditoria, diagnósticos e rollback de configuração.
 *
 * @param {Object} deps
 * @param {import("../../bootstrap.js").AppState} deps.state
 * @param {string}         deps.apiBase
 * @param {Function}       deps.fetchJson
 * @param {HTMLInputElement|null} deps.telemetryOptInEl
 * @param {(msg: string, opts?: object) => void} deps.showStatus
 * @param {(date: string) => string} deps.formatDateLabel
 * @param {() => Promise<void>} deps.onAfterConfigRollback
 * @param {typeof fetch}   [deps.fetchImpl=fetch]
 * @returns {{ loadTelemetryState, setTelemetryEnabled, showTelemetryStats, exportAuditLogsJson, exportDiagnosticsPackage, openConfigHistoryRollback }}
 */
