...
/**
 * ChatActionsController — CRUD de conversas (criar, renomear, duplicar, excluir, favoritar, arquivar, tags, prompt).
 *
 * @param {Object} deps
 ...
 * @param {(defaultTitle: string) => Promise<{title: string, userOnly: boolean}|null>} deps.openDuplicateModal
 * @param {() => string}  deps.uid                — gerador de UUID
 * @param {() => Promise<void>} deps.onLoadChats
 * @param {(chatId: string) => Promise<void>} deps.onSwitchChat
 * @returns {{ createNewChat, renameActiveChat, toggleFavoriteActiveChat, toggleArchiveActiveChat, editTagsActiveChat, editChatSystemPrompt, duplicateActiveChat, deleteActiveChat }}
 */
