...
/**
 * ChatNavigationController — carrega conversas, troca de aba, carrega mensagens.
 *
 * @param {Object} deps
 * @param {import("../../bootstrap.js").AppState} deps.state
 * @param {HTMLElement}   deps.chatEl             — container do histórico de mensagens
 * @param {HTMLElement}   deps.tabsEl             — container das abas desktop
 * @param {Function}      deps.fetchJson          — client HTTP
 * @param {() => string}  deps.buildChatsQueryString — monta querystring de filtros
 * @param {() => void}    deps.renderTabs         — re-renderiza lista de abas
 * @param {(role: string, text: string, opts?: object) => HTMLElement} deps.appendMessage
 * @param {() => void}    deps.hideTyping
 * @param {() => void}    deps.hideStatus
 * @param {(msg: string, opts?: object) => void} deps.showStatus
 * @param {() => Promise<void>} deps.loadRagDocuments
 * @param {(opts?: object) => Promise<void>} deps.runHistorySearch
 * @param {() => void}    deps.clearSearchResults
 * @param {{ createNewChat: Function }} deps.chatActionsController
 * @returns {{ loadChats: Function, loadMessages: Function, switchChat: Function, createNewChat: Function, navigateRelativeTab: Function }}
 */
