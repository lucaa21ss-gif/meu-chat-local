    ...
    const app = createApp({
        ...
        integrityService: {
            getOrRefresh: async () => ({
                ...
                checkedFiles: ["ops/scripts/local/install.sh"],
                mismatches: [],
                missingFiles: [],
    ...
    const app = createApp({
        ...
        integrityService: {
            getOrRefresh: async () => ({
                ...
                checkedFiles: ["ops/scripts/local/install.sh"],
                mismatches: [{ file: "ops/scripts/local/install.sh", reason: "hash-mismatch" }],
                missingFiles: [],
                lastCheckedAt: new Date().toISOString(),
                staleAfterMs: 30000,
            }),
    ...
    const app = createApp({
        ...
        capacityService: {
            getLatestSummary: async () => ({
                ...
                reportPath: "artifacts/capacity/capacity-report.json",
                thresholds: {
                    maxErrorRate: 5,
                    maxP95Ms: 1500,
                    minThroughputRps: 1,
...
test("POST /api/disaster-recovery/test executa cenário e retorna RTO/status final", async () => {
    const app = createApp({
        ...
        disasterRecoveryService: {
            runScenario: async () => ({
                ok: true,
                reportPath: "artifacts/dr/dr-test-ok.json",
                report: {
...
test("POST /api/disaster-recovery/test registra falha de restauracao", async () => {
    const app = createApp({
        ...
        disasterRecoveryService: {
            runScenario: async () => ({
                ok: false,
                reportPath: "artifacts/dr/dr-test-fail.json",
                report: {
...
test("upload policy: bloqueios de upload ficam registrados no audit log", async () => {
    ...
    test("approval: fluxo negado e expirado bloqueiam acao critica", async () => {
        ...
        const approvalsFile = path.join(__testDir, "..", "..", "..", "artifacts", "approvals", "operational-approvals.json");
