...
import { createIntegrityRuntimeService } from "../src/entrypoints/index.js";
...
test("integrity service: artefato integro retorna status ok", async () => {
    ...
    await mkdir(path.join(tempDir, "ops", "scripts", "local"), { recursive: true });

    const target = "ops/scripts/local/install.sh";
