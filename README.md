...
- Para diagnostico rapido de preflight/IO no CI, consulte o artifact `skill-governance-preflight-inspect` (`.json` e `.md`).
  ...
  O preflight aceita `--confidence-profile local|ci` para calibrar o score por contexto operacional (default: `local`).
...
- Exemplo de leitura do relatorio JSON com filtro de classes:
  ...
  - `npm run skill:governance:preflight:inspect:ci`
...
7. Se houver erro `Falha ao escrever output`, valide se o caminho passado em `--output` aponta para arquivo (e nao para diretorio) e se o processo tem permissao de escrita.
  Esse padrao vale para `skill-validator`, `skill-validator-schema-note`, `skill-schema-enforcement-status` e `skill-governance-preflight`.
