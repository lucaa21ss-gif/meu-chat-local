...
async function run() {
    const baseUrl = (getArg("--base-url", process.env.CANARY_BASE_URL || "http://localhost:3001") || "").replace(/\/$/, "");
    ...
    const outputPath = getArg("--output", path.join("server", "artifacts", "canary", "canary-report.json"));
