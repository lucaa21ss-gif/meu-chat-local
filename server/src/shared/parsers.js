...
export function parseChatImportPayload(rawBody = {}) {
  ...
  if (chat.messages.length > MAX_IMPORT_MESSAGES) {
    throw new HttpError(400, `Quantidade de mensagens excede o limite (${MAX_IMPORT_MESSAGES})`);
  }
  ...
  for (const item of chat.messages) {
    ...
    if (content.length > MAX_MESSAGE_LENGTH) {
      throw new HttpError(400, `Mensagem de importacao muito longa (max ${MAX_MESSAGE_LENGTH})`);
    }
    ...
    if (item.images !== undefined && item.images !== null) {
      ...
      if (item.images.length > MAX_IMAGES) {
        throw new HttpError(400, `Mensagem de importacao invalida: maximo de ${MAX_IMAGES} imagens`);
      }
...
export function parseBackupPayload(raw) {
  ...
  if (estimatedBytes > MAX_BACKUP_IMPORT_BYTES) {
    throw new HttpError(400, `Arquivo de backup excede limite de ${Math.floor(MAX_BACKUP_IMPORT_BYTES / (1024 * 1024))}MB`);
  }
  try {
...
export function parseBackupPassphrase(raw, { required = false } = {}) {
  const value = String(raw ?? "").trim();
  if (!value) {
    if (required) {
  ...
  if (value.length < MIN_BACKUP_PASSPHRASE_LENGTH) {
    throw new HttpError(400, `passphrase deve ter ao menos ${MIN_BACKUP_PASSPHRASE_LENGTH} caracteres`);
  }

  if (value.length > MAX_BACKUP_PASSPHRASE_LENGTH) {
    throw new HttpError(400, `passphrase muito longa (max ${MAX_BACKUP_PASSPHRASE_LENGTH})`);
  }
