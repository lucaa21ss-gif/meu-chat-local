...
function printUsage() {
    console.log(`Uso:
    node ops/scripts/local/capacity-profile.mjs [opcoes]

Opcoes:
  --base-url <url>            URL base da API (padrao: http://localhost:4000)
...
async function run() {
    ...
    const baseUrl = (getArg("--base-url", process.env.CAPACITY_BASE_URL || "http://localhost:4000") || "").replace(/\/$/, "");
    ...
    const outputPath = getArg("--output", path.join("artifacts", "capacity", "capacity-report.json"));
