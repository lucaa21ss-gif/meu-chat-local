...
usage() {
  cat <<'EOF'
Uso:
  bash ops/scripts/local/runbook-incident.sh --type <runbook-type> [opcoes]
...
Exemplos:
  bash ops/scripts/local/runbook-incident.sh --type model-offline --mode execute
  bash ops/scripts/local/runbook-incident.sh --type db-degraded --mode rollback
EOF
}
